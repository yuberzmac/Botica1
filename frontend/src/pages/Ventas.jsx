...
const Ventas = () => {
  ...
  const isSellerOrAdmin = user.rol === 'admin' || user.rol === 'vendedor';

  const fetchData = async () => {
    try {
      if (isSellerOrAdmin) {
        // Vendedores y Admins necesitan cargar productos y el historial de ventas
        ...
        // Clientes solo necesitan cargar los productos disponibles
  ...
  const procesarCompra = async () => {
    if (carrito.length === 0) {
      alert(isSellerOrAdmin ? 'La caja está vacía, no hay productos para vender' : 'El carrito está vacío');
      return;
    }
    ...
    try {
      ...
      alert(isSellerOrAdmin ? '¡Venta registrada con éxito!' : '¡Compra realizada con éxito!');
      setCarrito([]);
      setShowCarrito(false);
      fetchData(); // Actualizar stock disponible
    } catch (error) {
      // Extraer el motivo específico que nos envía el backend
      const serverError = error.response?.data;
      const errorDetalle = serverError?.details || serverError?.error || serverError?.message;
      const mensajeVisible = typeof errorDetalle === 'object' ? JSON.stringify(errorDetalle) : errorDetalle;
      
      alert(`Error al procesar: ${mensajeVisible || 'Revisa la consola para más detalles'}`);
      console.error('❌ Detalles del error de compra:', JSON.stringify(serverError, null, 2) || error.message);
    } finally {
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">
          {isSellerOrAdmin ? 'Punto de Venta' : 'Tienda - Comprar Productos'}
        </h1>
        <button
      ...
      {showCarrito && (
        <div className="fixed right-0 top-0 h-full w-96 bg-white shadow-lg border-l z-50 p-6 overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold">{isSellerOrAdmin ? 'Caja / Venta Actual' : 'Carrito de Compras'}</h2>
            <button
          ...
          {carrito.length === 0 ? (
            <p className="text-gray-500 text-center py-8">{isSellerOrAdmin ? 'No hay productos en la caja' : 'El carrito está vacío'}</p>
          ) : (
            <>
              ...
              <div className="border-t pt-4">
                ...
                >
                  <CreditCard size={20} />
                  {isSubmitting ? 'Procesando...' : (isSellerOrAdmin ? 'Registrar Venta' : 'Comprar Ahora')}
                </button>
              </div>
            </>
          )}
        ...
        {productos.map(producto => (
          <div key={producto.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div className="p-6">
              ...
              >
                <Plus size={16} />
                {producto.stock === 0 ? 'Sin Stock' : (isSellerOrAdmin ? 'Agregar a Venta' : 'Agregar al Carrito')}
              </button>
            </div>
          </div>
        ))}
      ...
      {/* Historial de ventas para admin y vendedor */}
      {isSellerOrAdmin && (
                ...
                {ventas.map(v => (
                  <tr key={v.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {v.vendedor_nombre ? v.vendedor_nombre : <span className="text-gray-400 italic">Venta Online</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {v.cliente_nombre ? v.cliente_nombre : <span className="text-gray-400 italic">Público General</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(v.fecha).toLocaleString()}</td>
                  </tr>
                ))}
                {ventas.length === 0 && (
                  <tr><td colSpan="7" className="px-6 py-4 text-center text-gray-500">No hay ventas registradas</td></tr>
                )}
              </tbody>
            </table>
          </div>
